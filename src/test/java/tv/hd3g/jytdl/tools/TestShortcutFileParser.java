...
public class TestShortcutFileParser extends TestCase {
	...
	public TestShortcutFileParser() throws MalformedURLException, IOException {
		f_regular_link_desktop = File.createTempFile("regular-link", ".desktop");
		FileUtils.copyInputStreamToFile(getClass().getResource("regular-link.desktop").openStream(), f_regular_link_desktop);

		f_video_link_desktop = File.createTempFile("video-link", ".desktop");
		FileUtils.copyInputStreamToFile(getClass().getResource("video-link.desktop").openStream(), f_video_link_desktop);
	...
	public void testDesktopFile() throws IOException {
		final ShortcutFileParser fp = new ShortcutFileParser();
